...
export const GuestGamify: React.FC = () => {
  ...
  const handleSpin = () => {
    ...
    setIsSpinning(true);
    analytics.track(ANALYTICS_EVENTS.SPIN_WHEEL_USED, {
  ...
  return (
    <GamifyContainer>
      ...
      <ProgressSection>
        <TierInfo>
          <TierIcon>{getTierIcon(gamification.tier)}</TierIcon>
          <div>
            <TierTitle>
              Tier {gamification.tier}: {getTierName(gamification.tier)}
            </TierTitle>
        ...
        <ProgressText>
          ...
          {gamification.tier < 3 &&
            ` (${100 - gamification.progress}% remaining)`}
        </ProgressText>
      </ProgressSection>
